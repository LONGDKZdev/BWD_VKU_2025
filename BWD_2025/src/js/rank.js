...
function publishPost() {
    ...
    const newPost = {
        ...
        user: {
            ...
            avatar: currentUser.avatar || 'src/images/default-avatar.jpg',
            rank: 0
        },
        category: category,
        content: postText,
    ...
    if (previewItems.length > 0) {
        const media = previewItems[0].querySelector('img, video');
        if (media) {
            newPost.media = {
    ...
    filteredPosts.unshift(newPost); // ➕ thêm vào danh sách lọc để không bị mất
    renderPosts();
...
document.getElementById('publish-btn-modal').onclick = function () {
    ...
    const newPost = {
        id: posts.length + 1,
        user: {
            id: currentUser.uid || 999, // hoặc dùng email làm ID nếu không có uid
            name: currentUser.name || "Không rõ",
            avatar: currentUser.avatar || "src/images/default-avatar.jpg",
            rank: 0 // bạn có thể thêm thứ hạng từ Firestore nếu cần
        },
...
function showComments(postId) {
    ...
    modalContent.innerHTML = `
        ...
        <div class="comment-form">
            <div class="user-avatar">
                <img src="${currentUser?.avatar || 'src/images/default-avatar.jpg'}" alt="${currentUser?.name || ''}">
        ...
        currentData.forEach((user, index) => {
            ...
            row.innerHTML = `
                ...
                <td class="user-column">
                    <div class="user-info">
                        <div class="user-avatar">
                            <img src="${user.avatar || 'src/images/default-avatar.jpg'}" alt="${user.name}">
                        </div>
...
loadUserInfo(user => {
    ...
    // Nếu chưa tồn tại user trong rankData, thêm mới
    if (!rankData.some(u => u.id === user.uid)) {
        rankData.push({
            id: user.uid,
            name: user.name,
            avatar: user.avatar || "src/images/default-avatar.jpg",
    ...
    // Tìm rank của currentUser từ danh sách đã sắp xếp
    ...
    // Gán hạng hiển thị ở explore.html
    const rankText = currentUser.rank ? `Hạng #${currentUser.rank}` : "Hạng #?";
    const rankSpan = document.querySelector(".rank-position");
    if (rankSpan) rankSpan.textContent = rankText;
